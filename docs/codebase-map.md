...
unreachable | interrupted`. Adding a variant forces an update in
...
| E2E (real) | `test/e2e/daemon.test.ts` | same | Starts its **own** `pueued` in a temp dir (own socket + state). Skips if pueue is not installed. |
...
No config file. Everything is flags plus two environment variables read by the
tool itself (`PUEUE_BINARY`, and `NO_COLOR`/`FORCE_COLOR` for output). Condition
